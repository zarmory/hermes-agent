...
def _build_switched_client(agent, new_provider, api_key, base_url, api_mode, new_norm) -> None:
    ...
    if api_mode == "anthropic_messages":
        ...
        # ``_build_anthropic_client_for_key``. Bedrock- and Vertex-hosted Claude
        # speak the Anthropic Messages protocol but authenticate through their
        # cloud's SDK, not an Anthropic API key. Without these branches a
        # ``/model`` switch rebuilt a direct Anthropic client pointed at the
        # cloud endpoint, so the SDK appended ``/v1/messages`` to a base_url
        # that has no such route and every post-switch call failed (404 on
        # Vertex).
