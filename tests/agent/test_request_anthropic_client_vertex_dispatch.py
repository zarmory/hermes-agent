...
def test_vertex_key_covers_project_region_and_timeout(agent):
    ...
    assert agent._request_anthropic_client_key() == base
    # Patch where the key function RESOLVES the symbol, not where AIAgent is
    # defined: _request_anthropic_client_key lives in agent.client_lifecycle,
    # which imports get_provider_request_timeout at module scope. Patching
    # run_agent's binding leaves the real timeout in place and the assertion
    # below silently compares a key to itself.
    with patch(
        "agent.client_lifecycle.get_provider_request_timeout", lambda *a, **kw: 999999
    ):
        assert agent._request_anthropic_client_key() != base
