...
def spies(monkeypatch):
    ...
    # Credential helpers resolve from agent.anthropic_credentials, not from
    # agent.anthropic_adapter — the adapter only re-exports them behind a
    # deprecation shim that upstream removes on 2026-09-14. Patching the shim
    # is a silent no-op (raising=False hides it) and breaks outright once the
    # re-export goes away.
    import agent.anthropic_credentials as creds_mod

    monkeypatch.setattr(creds_mod, "resolve_anthropic_token", lambda: "", raising=False)
    monkeypatch.setattr(creds_mod, "_is_oauth_token", lambda k: False, raising=False)
    return calls
